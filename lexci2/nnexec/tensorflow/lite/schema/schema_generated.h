...
struct QuantizationDetailsUnion {
  ...
  template <typename T>
  void Set(T&& val) {
    using FBRT = typename std::remove_reference<T>::type;
    Reset();
    type = QuantizationDetailsTraits<typename FBRT::TableType>::enum_value;
    if (type != QuantizationDetails_NONE) {
      value = new FBRT(std::forward<T>(val));
    }
  }
...
struct SparseIndexVectorUnion {
  ...
  void Set(T&& val) {
    using FBRT = typename std::remove_reference<T>::type;
    Reset();
    type = SparseIndexVectorTraits<typename FBRT::TableType>::enum_value;
...
struct BuiltinOptionsUnion {
  ...
  void Set(T&& val) {
    using FBRT = typename std::remove_reference<T>::type;
    Reset();
    type = BuiltinOptionsTraits<typename FBRT::TableType>::enum_value;

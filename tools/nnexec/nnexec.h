 ...
 *
 * Since C is an imperative language, the created object must be returned using
 * a `void*` pointer.
 *
 ...
 */
void* createNeuralNetwork(const uint8_t* ptr_nnData, size_t nnDataLen,
 ...
 *            virtually all neural networks have just a single input layer, this
 *            argument allows users to work with ones that have more.
 * @param[in] ptr_inputData Input data to set
 * @param[in] inputDataLen Input data length
 */
void setInputData(void* ptr_nn, size_t inputLayerIdx,
 ...
 * @param[in] outputLayerIdx Index (starting at 0) of the output to get. Though
 *            most neural networks have just a single output layer, this
 *            argument allows users to work with ones that have more.
 ...
 */
void getOutputData(void* ptr_nn, size_t outputLayerIdx, float* ptr_outputData,
